...
export const template = Template()
  ...
  .setWorkdir("/root")
  .runCmd("npx -y -p @ory/opencode ory-opencode install")
...
- What the template provides (harness CLI + @ory/opencode preinstalled, runtime
...
await sbx.commands.run("ory-opencode --version");  // sanity check
```
...
`Sandbox.create()` time, or run `npx -y -p @ory/opencode ory-opencode permissions enforce` inside a running
sandbox. Run `npx -y -p @ory/opencode ory-opencode permissions bootstrap` first on a fresh Ory project so
