...
`@ory/opencode` plugin ships two commands that
...
Both delegate to `npx -y -p @ory/opencode ory-opencode local <subcommand>`, so the same
...
```bash
npx -y -p @ory/opencode ory-opencode local up
```
...
`npx -y -p @ory/opencode ory-opencode local up --no-seed` instead.
...
```bash
npx -y -p @ory/opencode ory-opencode local configure
```
...
| Need                           | Command                                  |
|--------------------------------|------------------------------------------|
| Re-seed (after `local reset`)  | `npx -y -p @ory/opencode ory-opencode local seed`              |
| Tail Kratos logs               | `npx -y -p @ory/opencode ory-opencode local logs kratos -f`    |
| Check what's running           | `npx -y -p @ory/opencode ory-opencode local status`            |
| Print env vars to copy/paste   | `npx -y -p @ory/opencode ory-opencode local env`               |
...
`npx -y -p @ory/opencode ory-opencode local down`). Containers stop but volumes
...
```bash
npx -y -p @ory/opencode ory-opencode local reset
```
...
- **"Docker is not running"** — start Docker Desktop, then re-run
  `/ory:local-up`.
- **Gateway never becomes healthy** — `npx -y -p @ory/opencode ory-opencode local logs
  gateway` and `npx -y -p @ory/opencode ory-opencode local logs kratos` to see which service
