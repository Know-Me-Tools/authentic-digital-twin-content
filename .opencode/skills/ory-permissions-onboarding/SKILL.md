...
```sh
npx -y -p @ory/opencode ory-opencode configure --project-url <URL> --oauth2-client-id <CLIENT_ID> [--api-key <KEY>]
```
...
```sh
npx -y -p @ory/opencode ory-opencode permissions status
```
...
(next step) or run `npx -y -p @ory/opencode ory-opencode permissions observe` first to flip back to
non-blocking.
...
```sh
npx -y -p @ory/opencode ory-opencode permissions bootstrap
```
...
Re-run `npx -y -p @ory/opencode ory-opencode permissions status` afterwards to confirm coverage.
...
```sh
npx -y -p @ory/opencode ory-opencode permissions observe
```
...
If a tool is unexpectedly blocked, run `npx -y -p @ory/opencode ory-opencode permissions status` to
...
| Command | Effect |
|---|---|
| `npx -y -p @ory/opencode ory-opencode permissions status` | Print mode + per-tool allow/deny breakdown for the current user. Read-only. |
| `npx -y -p @ory/opencode ory-opencode permissions bootstrap` | Write `use` tuples for the harness's built-in tools. Idempotent. |
| `npx -y -p @ory/opencode ory-opencode permissions bootstrap --dry-run` | Print what would be written without making any changes. |
| `npx -y -p @ory/opencode ory-opencode permissions observe` | Persist `permissionMode = "observe"` (denies log, allow through). |
| `npx -y -p @ory/opencode ory-opencode permissions enforce` | Persist `permissionMode = "enforce"` (denies block). |
